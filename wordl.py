...
while tries <= tries_limit:
    ...
    for i,j in enumerate(guess):
        if j == word[i] and letter_count[j] >= letter_used_count[j] > 0:
            correct.append(j)
            result.append(f"{bcolors.GREEN}{j}{bcolors.ENDC}")
            num_of_correct +=1
        elif j in word and j not in misplaced and j not in correct and letter_count[j] >= letter_used_count[j] > 0:
            misplaced.append(j)
